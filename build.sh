...
do
    ...
    then
    	...
    	then
            ...
            eval "sed $reg_load -e \"s/^[[:space:]]*ecui.esr.loadClass(*//g\" -e \"s/');/.css'}\/\//g\" \"$file.js\"" | eval $tpl_proc | eval $css_proc > "../../$output/$file/$file.css"
	        eval "sed $reg_load -e \"s/^[[:space:]]*ecui.esr.loadClass(*//g\" -e \"s/');/.html'}\/\//g\" \"$file.js\"" | eval $tpl_proc | eval "sed $reg_comment" > "../../$output/$file/$file.html"
	        cd ../..
	    else
