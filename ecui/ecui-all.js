...
*/
    ...
    ui.Popup = {
        ...
        $pageresize: function (event) {
            this.$Popup.$pageresize.call(this, event);
            if (ui.Popup[this.getUID()].isShow()) {
        ...
        $pagescroll: function (event) {
            this.$Popup.$pagescroll.call(this, event);
...
(function () {
    ...
    function replace(rule) {
        return item.replace(/\$\{([^}]+)\}/g, function (match, name) {
            name = name.split('|');
            return context[name[0]] === null ? (name[1] || '') : context[name[0]];
        });
    }
                    ...
                    onsuccess: function (data) {
                        ...
                        if (!count) {
                            ...
                            if (err.length > 0 && onerror) {
                                if (onerror(url, context, arguments[0], arguments[1]) === false) {
                                    return;
                                }
                            }
                            onsuccess();
